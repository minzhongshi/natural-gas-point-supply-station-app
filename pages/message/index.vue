<template>
	<view>
		<view v-if="messageData.length > 0">
			<view v-for="item in messageData" key="item.id" class="consume_view" @click.native="messageGo(item)">
				<view class="wrap"><span class="consume_tip">{{item.name === '系统'? '系统消息':'推送消息'}}</span></view>
				<view class="desc">
					<view class="small_text">{{item.site}}<text>({{item.type}})</text></view>
...
<style lang="scss">
	...
	.consume_tip {
		...
		border: 1px dashed;
		box-shadow: 0 0 0 3px #E43D33, 0px 21px 5px -18px rgba(0, 0, 0, 0.6);
		background: #E43D33;
		font-size: 26rpx;
		color: #FFFFFF;
	}
